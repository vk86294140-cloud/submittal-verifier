...
def test_clears_and_recurs():
    ...
    round2 = _report(
        "Test report enclosed. Pile height: 0.30 inch. Tested to ASTM E84."
    )
...
def test_resolved_all_when_clean():
    ...
    prior = to_dict(round1)["findings"]
    fixed = _report(
        "Test report enclosed. Pile height: 0.30 inch. Complies with ASTM E662."
    )
...
def test_render_lists_buckets():
    d = diff_reviews(to_dict(_report("Product data only."))["findings"],
                     _report("Product data only."))
    text = render(d)
